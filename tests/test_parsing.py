...
async def test_parses_clean_json():
    p = FakeProvider(['{"x": 1}'])
    out, usage = await complete_json(p, MSGS, config=CFG, schema=Out)
    assert out.x == 1
...
async def test_strips_code_fence():
    p = FakeProvider(['```json\n{"x": 2}\n```'])
    out, _ = await complete_json(p, MSGS, config=CFG, schema=Out)
    assert out.x == 2
...
async def test_retries_once_then_succeeds():
    ...
    assert usage["total_tokens"] == 30  # both calls billed (15 + 15)
